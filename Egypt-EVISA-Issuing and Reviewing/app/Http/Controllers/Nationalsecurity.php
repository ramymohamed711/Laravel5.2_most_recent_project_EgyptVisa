...
class Nationalsecurity extends Controller
{
 ...
 use CentralVisaFunctions ; 
          ...
          */
          ...
          public function nationalsecurity(){

            return view('nationalsecurity.nationalsecurity') ;
          }
          ...
          */
          public function applications(){
            ...
            if($applications->isEmpty())   {
             ...
             $applications =   Applicationlog::where("APPSTATUS_ID","=",'2')
             ->whereNull('ACTION_USER')
             ->whereNotExists(function($query)
             {
              $query->select(DB::raw('APPLICATION_NO'))
              ->from('UNDERSUSPEND')
              ->whereRaw('application_log.APPLICATION_NO = UNDERSUSPEND.APPLICATION_NO AND APPSTATUS_ID = 8 ');
            })
             ->whereNotExists(function($query)
             {
               $query->select(DB::raw('APPLICATION_NO'))
             ...
             })->whereNotExists(function($query)
             {
               $query->select(DB::raw('APPLICATION_NO'))
               ->from('UNDERPERMIT')
               ->whereRaw('application_log.APPLICATION_NO = UNDERPERMIT.APPLICATION_NO AND APPSTATUS_ID IN(11,12)');
             })
             ->orderBy("log_date")
             ->take(10)
             ->get();
          ...
          return view('nationalsecurity.applications',['applications'=>$applications,'numbers'=>base64_encode(urlencode($numbers))]) ;
          ...
          */
          ...
          public function getsimilarities($application_no){
            ...
            ->where(function($query){
              $query->where("APPSTATUS_ID","=",2)
              ->where("ACTION_USER",'=',Auth::user()->id);
            })
            ->orWhere(function($query){
              $query->where("APPSTATUS_ID","=",8)
           ...
           return view('nationalsecurity.similarities',['application'=>$application,'similarities'=>$similarities]);
         }
          ...
          */
          public function search(){
            return view('nationalsecurity.search');
          }
          /**
          ...
          */
          public function handelsearch(Request $request){
            ...
            return view('nationalsecurity.similarities',['application'=>$application,'similarities'=>$similarities,'search'=>true]);
          }
          ...
          */
          ...
          //$applications = (object)$applications ;
            return view('nationalsecurity.printapplications',['applications'=>$applications,'print'=>true, 'error'=>$error]);
          }
          else{
           return "Error";
         }
         ...
         public function rejectapplication(Request $request){
          ...
          if(!empty($application)){
            ...
            if(Applicationlog::create($data)){
             ...
             ->where('APPSTATUS_ID','=',14)->first();
             $only_security = Applicationlog::where('application_no','=',$request->application_no)
             ->where('APPSTATUS_ID','=',3)->first();
             if(!empty($application) || empty($only_security)){
               $data = ['application_no'=>$request->application_no , 
               'user_id' => Auth::user()->id,
               'appstatus_id' => 9,
               'log_date'=>Carbon\Carbon::now('Africa/Cairo')->format('Y-M-d h.i.s A')
      ...
      */
        public function loadprinted(){
          return view('nationalsecurity.loadprinted') ;
        }
      ...
      */
      public function printagain(Request $request){
        ...
        DB::setDateFormat('yyyy-MM-dd hh.mi.ss am');
        $applications =  Applicationlog::where('APPSTATUS_ID','=','8')
        ->where('USER_ID','=',Auth::user()->id)
        ->whereNotExists(function($query)
        {
          $query->select(DB::raw('application_no'))
        ...
        return view('nationalsecurity.printagain',['applications'=>$applications , 'numbers'=>base64_encode(urlencode($numbers))]);
